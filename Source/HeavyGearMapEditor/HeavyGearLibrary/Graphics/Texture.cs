...
namespace HeavyGear.Graphics
{
    ...
    public class Texture : IDisposable
    {
        ...
        public void RenderOnScreen(Rectangle destinationRect, Rectangle sourceRect)
        {
            alphaSprite.Draw(internalXnaTexture, destinationRect, sourceRect, Color.White);
        ...
        public void RenderOnScreen(Rectangle destinationRect,
            int pixelX, int pixelY, int pixelWidth, int pixelHeight)
        {
            alphaSprite.Draw(internalXnaTexture, destinationRect, new Rectangle(pixelX, pixelY,
        ...
        public void RenderOnScreen(Rectangle rect, Rectangle? pixelRect,
            Color color, float rotationValue)
        {
            SpriteEffects spriteEffects = SpriteEffects.None;
            if ((int)rotationValue >= 180)
            {
                rotationValue -= 180;
            ...
            if (pixelRect != null)
                origin = new Vector2(((Rectangle)pixelRect).Width / 2, ((Rectangle)pixelRect).Height / 2);
            else
                origin = new Vector2(Width / 2, Height / 2);

            alphaSprite.Draw(internalXnaTexture, rect, pixelRect, color, rotationValue, origin, spriteEffects, 0);
        }

        /// <summary>
        /// Overload for hex grids
        /// </summary>
        ...
        public void RenderSpriteOnScreen(Vector2 position, Rectangle ?sourceRectangle,
            Color color, float rotationValue, Vector2 origin)
        {
            SpriteEffects spriteEffects = SpriteEffects.None;
            if ((int)rotationValue >= 180)
            {
            ...
            Rectangle renderRect = new Rectangle(
                (int)position.X, 
                (int)position.Y, 
                (int)(BaseGame.TileWidth / zoomValue), 
                (int)(BaseGame.TileHeight / zoomValue)
                );

            alphaSprite.Draw(internalXnaTexture, renderRect, sourceRectangle, color, rotationValue, origin, //scale,
                    spriteEffects, 0.01f);
        }

        public void RenderSpriteOnScreen(Rectangle renderRect, Rectangle? sourceRect, Color color, float rotationValue)
        {
            SpriteEffects spriteEffects = SpriteEffects.None;
            ...
            Vector2 origin = Vector2.Zero;
            if (sourceRect != null)
                origin = new Vector2(((Rectangle)sourceRect).Width / 2, ((Rectangle)sourceRect).Height / 2);
            else
                origin = new Vector2(Width / 2, Height / 2);
            alphaSprite.Draw(internalXnaTexture, renderRect, sourceRect, color, rotationValue, origin, spriteEffects, 0);
        }
        ...
        /// <param name="scale"></param>
        /// <param name="layerDepth">0 front, 1 back</param>
        public void RenderTileOnScreen(Vector2 position, Rectangle ?sourceRectangle, Color color, Vector2 origin, float layerDepth)
        {
            alphaSprite.Draw(internalXnaTexture, position, sourceRectangle, color, 0, origin, 1.0f,
